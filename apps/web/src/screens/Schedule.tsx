...
import { Clock, Scissors, Lock, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Schedule() {
  ...
  const [shopStatus, setShopStatus] = useState<{ is_open: boolean; shop_name: string } | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      getShopStatus().then((s) => setShopStatus(s)).catch(() => {}),
        ...
        {!isShopOpen && (
          <Card className="border-red-200 bg-red-50">
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <div>
                <p className="font-semibold text-red-700">Shop is currently closed</p>
                <p className="text-sm text-red-600">
                  We are not accepting new bookings at this time. Existing appointments are still honored.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardContent className="flex items-center justify-between py-3">
            ...
            <div className="text-center">
              <p className="font-medium text-sm">{formatWeekRange(weekStart)}</p>
              <Button
                variant="ghost"
                size="sm"
              ...
              {days.map((day) => (
                <Card key={day.date} className="flex flex-col">
                  ...
                  <CardContent className="flex-1 pt-0 space-y-1.5">
                    {day.slots.length === 0 ? (
                      <p className="text-xs text-muted-foreground text-center py-4">Closed</p>
                    ) : (
                      day.slots.map((slot) => (
                        <button
                          key={slot.time}
                          onClick={() => {
                            if (slot.status === "available" && isShopOpen) handleSlotClick(slot.time, day.date);
                          }}
                ...
                <Card>
                  ...
                  <CardContent className="space-y-1.5">
                    {mobileDay.slots.length === 0 ? (
                      <p className="text-xs text-muted-foreground text-center py-4">Closed — no availability</p>
                    ) : (
                      mobileDay.slots.map((slot) => (
                        <button
                          key={slot.time}
                          onClick={() => {
                            if (slot.status === "available" && isShopOpen) handleSlotClick(slot.time, mobileDay.date);
                          }}
                          disabled={slot.status !== "available" || !isShopOpen}
